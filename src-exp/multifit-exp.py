...
def calculate(x, y, x1, y1, file, type, ymax, drawMap, showPlot):
    ...
    pars['p0_amplitude'].set(inv[8,1]*(ymax*50), min=inv[9,1], max = inv[10,1])
    if type ==0:
    ...
    for i in range (1, defPar.NumPeaks):
        if fpeak[i]!=0:
            ...
            pars['p{:}_amplitude'.format(str(i))].set(inv[8,i+1]*(ymax*50), min=inv[9,i+1], max = inv[10,i+1])
            if type ==0:
        ...
        if o in ("-b" , "--batch"):
            ...
            if(defPar.multiproc == True):
                ...
                for f in glob.glob('*.txt'):
                    if (f != 'summary.txt'):
                        ...
                        p.apply_async(calculate, args=(rs.x, rs.y, '0', '0', f, type, rs.ymax, False, False))
                p.close()
                p.join()
            else:
                for f in glob.glob('*.txt'):
                    if (f != 'summary.txt'):
                        rs = readSingleSpectra(f)
                        calculate(rs.x, rs.y, '0', '0', f, type, rs.ymax, False, False)
        
        elif o in ("-f", "--file"):
            file = str(sys.argv[2])
            type = int(sys.argv[3])
            rs = readSingleSpectra(file)
            calculate(rs.x, rs.y, '0', '0', file, type, rs.ymax, False, True)
            ...
            if(defPar.multiproc == True):
                ...
                for i in range (1, rm.num_lines):
                    p.apply_async(calculate, args=(rm.x, rm.y[i], rm.x1[i], rm.y1[i], file, type, rs.ymax[i], True, False))
                p.close()
                p.join()
                ...
                for i in range (1, rm.num_lines):
                    calculate(rm.x, rm.y[i], rm.x1[i], rm.y1[i], file, type, rs.ymax[i], True, False)
                #map.draw(os.path.splitext(file)[0] + '_map.txt', True)
